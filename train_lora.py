...
def run_pilot(args):
    ...
    train_df = load_manifest("train", accent=getattr(args, "accent", None))
    val_df = load_manifest("val", accent=getattr(args, "accent", None)).iloc[:64].reset_index(drop=True)

    pilot_dir = PROJECT_ROOT / "runs" / "pilot_tmp"
...
def train_full(args, batch_size: int, grad_accum: int, epochs: int):
    ...
    train_df = load_manifest("train", accent=getattr(args, "accent", None))
    val_df = load_manifest("val", accent=getattr(args, "accent", None))
    ...
    if state.best_model_checkpoint:
        try:
            best_epoch = round(state.epoch, 2)
        except Exception:
            pass
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="vietnamese",
        choices=["vietnamese", "arabic", "chinese", "hindi", "korean", "spanish"],
