import argparse
from s2t.datasets.cmu_arctic import CMU_ACCENT_SPEAKERS, CMU_SPEAKERS, speaker_dir
import json
import os
import random
...
def get_speaker_dir(spk: str) -> Path:
    if spk in CMU_SPEAKERS:
        return speaker_dir(spk)
...
def _target_data_dir(accent: str, speaker: str | None = None) -> Path:
    if speaker is not None:
        return get_speaker_data_dir(speaker, accent=accent)
    return DATA_DIR if accent.lower() == "vietnamese" else DATA_DIR / accent.lower()


def prepare_l2_arctic_for_accent(
    accent: str = "vietnamese",
    force: bool = False,
    speaker: str | None = None,
):
    speakers = ACCENT_SPEAKERS[accent.lower()]
    if speaker is not None:
        speakers = [normalize_speaker(speaker, accent=accent)]
    accent_data_dir = _target_data_dir(accent, speaker)
    ...
    for spk in speakers:
        ...
        if not txt_files:
            raise ValueError(f"Missing transcripts for required speaker {spk}: {spk_txt_dir}")
    ...
    for spk in speakers:
        ...
        for sid in spk_ids[spk]:
            ...
            if not raw_wav_path.exists() or not txt_path.exists():
                raise ValueError(f"Missing audio/transcript pair: {spk}/{sid}")
            out_wav_path = spk_wav16k_dir / f"{sid}.wav"
            shared_wav_path = DATA_DIR / "wav16k" / spk / f"{sid}.wav"
            if speaker is not None and shared_wav_path.exists() and not force:
                out_wav_path = shared_wav_path
...
def prepare_suitcase_for_accent(
    accent: str = "vietnamese",
    force: bool = False,
    speaker: str | None = None,
):
    accent_speakers = ACCENT_SPEAKERS[accent.lower()]
    if speaker is not None:
        accent_speakers = [normalize_speaker(speaker, accent=accent)]
    speakers = [s.lower() for s in accent_speakers]
    accent_data_dir = _target_data_dir(accent, speaker)
    ...
    for spk in speakers:
        ...
        for k, chunk_words in enumerate(chunks):
            ...
            shared_chunk_file = DATA_DIR / "wav16k" / "suitcase" / f"{spk}_{k}.wav"
            if speaker is not None and shared_chunk_file.exists() and not force:
                chunk_wav_file = shared_chunk_file
...
def prepare_accent(
    ...
    print(f"\n==========================================")
    scope = f"{accent.upper()} / {speaker.upper()}" if speaker else accent.upper()
    print(f"   PREPARING DATA FOR: {scope}   ")
    print(f"==========================================")
    if speaker is not None:
        speaker = normalize_speaker(speaker, accent=accent)
    accent_data_dir = _target_data_dir(accent, speaker)
    accent_manifest = accent_data_dir / "manifest.csv"

    l2_rows = prepare_l2_arctic_for_accent(accent, force=force, speaker=speaker)
    suitcase_rows = [] if accent in CMU_ACCENT_SPEAKERS else prepare_suitcase_for_accent(accent, force=force, speaker=speaker)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str.upper,
        default=None,
        help="Prepare only one speaker (for Vietnamese: HQTV, PNV, THV, or TLV)",
    )
    args = parser.parse_args()

    if args.accent == "all":
        if args.speaker:
            parser.error("--speaker cannot be combined with --accent all")
        for acc in ACCENT_SPEAKERS.keys():
            prepare_accent(acc, force=args.force)
    else:
        try:
            prepare_accent(args.accent, force=args.force, speaker=args.speaker)
