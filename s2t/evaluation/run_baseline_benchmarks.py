...
from s2t.evaluation.evaluate_wer import evaluate_model_on_split
...
def evaluate_model_for_accent(model, processor, split_name, accent, batch_size=16, device="cuda") -> Dict:
    """Use the main evaluator while retaining the baseline report's gender labels."""
    result = evaluate_model_on_split(
        model=model, processor=processor, split_name=split_name,
        accent=accent, batch_size=batch_size, device=device,
    )
    result["gender_results"] = {}
    for label, speakers in [("Male Avg", MALE_SPEAKERS), ("Female Avg", FEMALE_SPEAKERS)]:
        samples = [s for s in result["samples"] if s["speaker"].upper() in speakers]
        if samples:
            refs = [s["ref_norm"] for s in samples]
            hyps = [s["hyp_norm"] for s in samples]
            result["gender_results"][label] = {
                "wer": round(corpus_wer(refs, hyps) * 100, 2),
                "cer": round(corpus_cer(refs, hyps) * 100, 2),
                "n_utts": len(samples),
            }
    return result
